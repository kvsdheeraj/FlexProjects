...
CFLAGS = -Os -Wall -Wextra -g -std=c99 -D_DEFAULT_SOURCE
#CFLAGS = -Os -Wall -Wextra -Werror -std=c99 -D_DEFAULT_SOURCE -Wstrict-prototypes -Wmissing-prototypes -fms-extensions 
ifeq ($(CC),clang)
	CFLAGS += -Wno-microsoft-anon-tag
endif
DEPFLAGS = -MT $@ -MMD -MP -MF $(DEPENDENCYDIR)/$*.d

EXE = zForceSDK-Example
SRCS = zForceSDK-Example.c ErrorString.c DumpMessage.c gen2test.c
INCLUDES = -I$(INCLUDEDIR) -I$(ZFORCESDKDIR)
LIBS = -L./zForceSDK/Linux/$(ARCHITECTURE) -lzForce -lm -pthread -ludev -Wl,-rpath='$$ORIGIN/zForceSDK/Linux/$(ARCHITECTURE)'
